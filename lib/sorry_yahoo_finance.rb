...
class SorryYahooFinance
      ...
      if code.class == Fixnum && code.to_s.size == 4
        begin
          if date
            ...
            get_infos("http://info.finance.yahoo.co.jp/history/?code=#{code}.T&sy=#{year}&sm=#{month}&sd=#{day}&ey=#{year}&em=#{month}&ed=#{day}&tm=d")
          else
            get_infos("http://stocks.finance.yahoo.co.jp/stocks/detail/?code=#{code}")
          end
        rescue => ex
  ...
  def initialize(code, date=nil)
    @values = self.class.get(code, date)
  end
...
class Get < SorryYahooFinance; end